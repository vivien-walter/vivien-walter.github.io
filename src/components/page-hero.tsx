...
function PageHero({ title, introduction, eyebrow, breadcrumbs, actions, footer, image }: PageHeroProps) {
  ...
  return (
    <section className="border-border bg-brand-hero border-b" aria-labelledby="page-title">
      <div className={cn('max-w-editorial mx-auto grid w-full', 'lg:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.9fr)]')}>
        <div className={cn('px-page min-w-0', 'pt-5 pb-8', 'sm:pt-6 sm:pb-10', 'lg:pt-6 lg:pb-12', 'xl:pb-14')}>
          <div className="min-h-6">{breadcrumbs ? <Breadcrumbs ariaLabel={breadcrumbs.ariaLabel} items={breadcrumbs.items} /> : null}</div>

          <header className="max-w-readable mt-4 sm:mt-5">
            {eyebrow ? (
              <p className={cn('!m-0', 'font-mono text-xs font-semibold uppercase', 'text-brand-accent tracking-[0.12em]')}>{eyebrow}</p>
            ) : null}
          ...
          {footer ? <div className={cn('border-border mt-10 border-t pt-6', 'sm:mt-12 sm:pt-8')}>{footer}</div> : null}
        </div>

        <div
          className={cn(
            'min-w-0',
            image ? ['bg-brand-background min-h-64', 'sm:min-h-80 lg:min-h-full', 'lg:border-border lg:border-l'] : 'hidden lg:block',
          )}
