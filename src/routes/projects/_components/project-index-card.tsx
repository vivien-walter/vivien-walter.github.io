...
function formatProjectYears(
  period: ProjectIndexCardContent["period"],
): string | undefined {
  if (!period) {
    return undefined;
  }
...
function ProjectIndexCard({
  ...
  const headingId =
    `project-index-${project.id}-title`;

  const period = formatProjectYears(
    project.period,
  );
  ...
  const programmingLanguageSet = new Set(
  ...
  const visibleTechnologies = (
    ...
    .filter(
      (technology) =>
        !programmingLanguageSet.has(technology),
    )
      ...
      >
        ...
        >
          ...
          {hasMetadata ? (
            <dl
              className={cn(
                "m-0 grid content-start gap-6",
                "border-t border-border",
                "bg-brand-hero/45 p-5",
