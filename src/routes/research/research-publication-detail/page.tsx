...
} from "./_components/publication-related-items-section";
...
function getPublicationResources(
  publication: object,
): readonly PublicationResource[] | undefined {
  if (
    !("resources" in publication) ||
    !Array.isArray(publication.resources)
  ) {
    return undefined;
  }

  return publication.resources as
    readonly PublicationResource[];
}

function getPublicationSoftwareIds(
  publication: object,
): readonly string[] {
  if (
    !("softwareIds" in publication) ||
    !Array.isArray(publication.softwareIds)
  ) {
    return [];
  }

  return publication.softwareIds.filter(
    (softwareId): softwareId is string =>
...
function ResearchPublicationDetailPage() {
  ...
  const resources =
    getPublicationResources(
      publication,
    );

  const softwareIds =
    getPublicationSoftwareIds(
      publication,
    );

  const relatedThemes =
    publication.themeIds.flatMap(
      (themeId) => {
        const theme =
          getResearchThemeById(
            language,
  ...
  const relatedProjects =
    publication.projectIds.flatMap(
      (projectId) => {
  ...
  const relatedExperiences =
    publication.experienceIds.flatMap(
      (experienceId) => {
        const experience =
          getExperienceById(
            language,
  ...
  const relatedGroups:
    readonly PublicationRelatedGroup[] = [
      {
        id: "themes",
        title:
  ...
  const hasDescription =
    publication.description !==
      undefined &&
    publication.description.paragraphs
      .length > 0;
...
>
        <section
          ...
          aria-labelledby={`${idPrefix}-metadata-title`}
        >
          <SectionHeader
            title={
              page.publicationDetail
            ...
            titleId={`${idPrefix}-metadata-title`}
            className="mb-8"
          />
