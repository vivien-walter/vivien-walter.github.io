...
type FollowActivityResource = FollowActivitiesSectionContent['resources'][number];

interface FollowActivityItemProps {
  readonly resource: FollowActivityResource;
}

function FollowActivityItem({ resource }: FollowActivityItemProps) {
  ...
  return (
    <li className="m-0 min-w-0">
      ...
      >
        <InteractiveCard interaction="group" className="border-border-strong bg-brand-background h-full min-h-40 gap-0 rounded-lg py-0 shadow-none">
    ...
    <Section containerClassName="py-12 sm:py-14 lg:py-16" aria-labelledby="software-activities-title">
      ...
      <ul className="m-0 grid list-none gap-5 p-0 sm:grid-cols-2 lg:grid-cols-3">
        {content.resources.map((resource) => (
          <FollowActivityItem key={resource.id} resource={resource} />
        ))}
      </ul>
    </Section>
  );
