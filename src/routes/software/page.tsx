...
function getSoftwareTags(software: SoftwareCatalogSource): readonly string[] {
  return Array.from(new Set([...software.languages, ...software.technologyGroups.flatMap((group) => group.items.map((item) => item.label))]));
}

function compareOptionalYears(firstYear: number | undefined, secondYear: number | undefined, direction: 'ascending' | 'descending'): number {
  ...
  return direction === 'ascending' ? firstYear - secondYear : secondYear - firstYear;
}
...
function SoftwarePage() {
      ...
      .sort((first, second) => {
        ...
        switch (sortBy) {
          case 'year-descending':
            comparison = compareOptionalYears(first.software.year, second.software.year, 'descending');
            break;

          case 'year-ascending':
            comparison = compareOptionalYears(first.software.year, second.software.year, 'ascending');
            break;
  ...
  const featuredSoftware = page.featuredSoftwareIds.flatMap((softwareId) => {
    const entry = softwareItems.find(({ software }) => software.id === softwareId);
  ...
  const catalogPanelId = `software-catalog-${activeKind}-panel`;

  const emptyState = activeKind === 'software' ? page.catalog.emptyStates.software : page.catalog.emptyStates.webApplications;
    ...
    <div className="overflow-hidden">
      ...
      {featuredSoftware ? (
        <Section className="border-border border-b" containerClassName="py-12 sm:py-14 lg:py-16" aria-labelledby="featured-software-title">
          ...
          <Card className={cn('gap-0 overflow-hidden', 'rounded-lg py-0', 'border-border-strong', 'bg-brand-background', 'shadow-subtle')}>
            <article
              className={cn('grid min-w-0', 'md:grid-cols-[10rem_minmax(0,1fr)]')}
              aria-labelledby={`featured-software-${featuredSoftware.id}`}
            >
              <div className={cn('flex min-h-36', 'items-center justify-center', 'bg-action-soft', 'text-brand-primary', 'md:min-h-full')}>
                <featuredSoftware.icon aria-hidden="true" className="size-14" weight="regular" />
              </div>

              <div className="min-w-0 p-5 sm:p-7">
                <Badge
                  variant="outline"
                  className={cn('mb-4', 'border-brand-primary/35', 'bg-brand-background', 'font-mono text-xs', 'font-semibold', 'text-brand-primary')}
                >
                ...
                <h3
                  ...
                  className={cn('!m-0 text-xl', 'font-bold', 'leading-heading', 'tracking-[-0.025em]', 'text-brand-ink')}
                >
                  {featuredSoftware.title}
                </h3>

                <p className={cn('!mt-4 !mb-0', 'max-w-readable', 'text-muted-foreground')}>{featuredSoftware.summary}</p>

                <ul className={cn('!mt-5 !mb-0 flex', 'list-none flex-wrap', 'gap-2 !p-0')} aria-label={page.catalog.technologiesLabel}>
                  {getSoftwareTags(featuredSoftware)
                    .slice(0, 8)
                    .map((tag) => (
                      <li key={tag} className="!m-0">
                        <Badge
                          variant="secondary"
                          className={cn('border', 'border-border', 'bg-brand-hero', 'px-3 py-1', 'font-mono', 'font-medium', 'text-muted-foreground')}
                        >
                          {tag}
                        </Badge>
                      </li>
                ...
                <div className={cn('mt-6 flex flex-wrap', 'items-center', 'gap-x-5 gap-y-3', 'border-t', 'border-border pt-6')}>
      ...
      <div className={cn('mx-auto w-full', 'max-w-editorial px-page', 'pb-12 sm:pb-14 lg:pb-16')}>
        <Section contained={false} className="py-12 sm:py-14 lg:py-16" aria-labelledby="software-catalog-title">
          <SectionHeader className="mb-8 sm:mb-10">
            <SectionTitle id="software-catalog-title">{page.catalog.sectionTitle}</SectionTitle>
          </SectionHeader>

          <div className={cn('flex flex-col', 'lg:flex-row', 'lg:items-end', 'lg:justify-between')}>
            <div
              role="tablist"
              aria-label={page.catalog.sectionTitle}
            ...
            >
              <Button
                ...
                onClick={() => {
                  setActiveKind('software');
                }}
              >
                {page.catalog.tabs.software}
              </Button>
              ...
              <Button
                ...
                onClick={() => {
                  setActiveKind('web-application');
                }}
              >
                {page.catalog.tabs.webApplications}
              </Button>
            </div>
          ...
          <div
            id={catalogPanelId}
            role="tabpanel"
            aria-labelledby={activeKind === 'software' ? articlesTabId : webApplicationsTabId}
            className="mt-3 sm:mt-4"
          >
            {visibleSoftware.length > 0 ? (
              <div className="grid gap-5" aria-label={page.catalog.listLabel}>
                {visibleSoftware.map(({ software }) => {
                  const catalogEntry: SoftwareCatalogEntryContent = {
                    id: software.id,
                    icon: software.icon,
                    title: software.title,
                    summary: software.summary,
                    kindLabel: page.kindLabels[software.kind],
                    tags: getSoftwareTags(software),
                  };

                  return (
                    <SoftwareCatalogEntry
                      key={software.id}
                      software={catalogEntry}
                      language={language}
                      technologiesLabel={page.catalog.technologiesLabel}
                      viewLabel={page.catalog.actions.viewSoftware}
                    />
                  );
                })}
              </div>
        ...
        {page.resources.length > 0 ? (
          <Section contained={false} className="border-border border-t py-12 sm:py-14 lg:py-16" aria-labelledby="software-activities-title">
            <SectionHeader className="mb-8 sm:mb-10">
              <SectionTitle id="software-activities-title">{page.followActivities.title}</SectionTitle>
            </SectionHeader>
