...
type ParallelActivityItem = {
  readonly activityId: ExperienceParallelActivityId;
  readonly activity: ExperienceParallelActivityContent;
};

type ParallelActivitiesSectionProps = {
  readonly title: string;
  readonly items: readonly ParallelActivityItem[];
  readonly language: SupportedLanguage;
  readonly actionLabel: string;
};

const iconByActivityId: Readonly<
  Record<ExperienceParallelActivityId, Icon>
> = {
  consulting: BriefcaseIcon,
  "scientific-research": MicroscopeIcon,
    ...
    >
      ...
      >
        {items.map(({ activity, activityId }) => {
          const ActivityIcon = iconByActivityId[activityId];
          const titleId = `parallel-activity-${activityId}-title`;

          return (
            <li className="m-0 min-w-0" key={activityId}>
              <Card
                className={cn(
              ...
              >
                ...
                <CardFooter className="px-6 pt-5 pb-6">
                  ...
                  >
                    <Link
                      aria-labelledby={`${titleId} parallel-activity-${activityId}-action`}
                    ...
                    >
                      <span
                        id={`parallel-activity-${activityId}-action`}
                      >
                        {actionLabel}
                      </span>
