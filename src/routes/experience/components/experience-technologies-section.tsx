...
import type {
  ExperienceTechnologyGroup,
  ExperienceTechnologyItem,
} from "../data/experience-content.types";

type ExperienceTechnologiesSectionProps = {
  readonly externalLinkLabel: string;
  readonly groups?: readonly ExperienceTechnologyGroup[];
  readonly idPrefix: string;
  readonly title: string;
};

type TechnologyDescriptionBadgeProps = {
  readonly item: ExperienceTechnologyItem;
...
function ExperienceTechnologiesSection({
  externalLinkLabel,
  groups,
  idPrefix,
  title,
}: ExperienceTechnologiesSectionProps) {
    ...
    >
      ...
      <div className="grid gap-8">
        {populatedGroups.map((group, groupIndex) => {
          const groupTitleId =
            `${idPrefix}-technology-group-${groupIndex + 1}`;
            ...
            >
              ...
              >
                {group.items.map((item, itemIndex) => {
                  const itemKey =
                    `${groupIndex}-${itemIndex}-${item.label}`;
...
export default ExperienceTechnologiesSection;