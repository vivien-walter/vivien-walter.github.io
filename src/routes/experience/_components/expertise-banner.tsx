...
export default function ExpertiseBanner({ items }: ExpertiseBannerProps) {
  return (
    <div className="bg-brand-primary text-white">
      <ul className="max-w-editorial px-page mx-auto grid w-full list-none gap-x-8 gap-y-10 py-9 sm:py-10 md:grid-cols-2 lg:grid-cols-3 lg:gap-x-10 lg:py-12">
        {items.map((item) => {
          ...
          return (
            <li key={item.id} className="m-0 grid min-w-0 content-start gap-4">
              ...
              <div className="min-w-0 text-left">
                <h2 className="!leading-heading !m-0 text-center !text-base !font-bold !tracking-[-0.01em] !text-white">{item.title}</h2>

                <p className="leading-body !mt-2 !mb-0 text-sm text-white/85">{item.description}</p>
              </div>
            </li>
          );
        })}
